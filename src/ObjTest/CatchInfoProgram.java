...
/**
 * 获取信息小程序——基本功能简单实现版 启动程序后，输入网址并选择需要获取的信息模式，
...
public class CatchInfoProgram {
	...
	public static void start() {
		try {
			System.out.println("程序启动，请输入网址：");
			Scanner sc = new Scanner(System.in);
			String webAdress = sc.nextLine();
			System.out.println("请选择模式：（1-4）");
			int modeSelect = sc.nextInt();

			switch (modeSelect) {
			case 1:
				System.out.println("已选择获取邮箱地址方法。正在读取网站信息……");
				catchEmail(enterWebside(webAdress));
				break;

			case 2:
				System.out.println("已选择获取手机号码方法。正在读取网站信息……");
				catchMobilePhone(enterWebside(webAdress));
				break;

			case 3:
				System.out.println("已选择获取固定电话方法。正在读取网站信息……");
				catchPhoneNumber(enterWebside(webAdress));
				break;

			case 4:
				System.out.println("已选择获取网址信息方法。正在读取网站信息……");
				catchWebside(enterWebside(webAdress));
				break;

			default:
				System.out.println("错误未知的输入！请输入1-4以内的数字。程序结束。");
				sc.close();
				System.exit(1);
			}
	...
	public static BufferedReader enterWebside(String webAdress) {
		try {
			...
			e.printStackTrace();
			System.out.println("发生异常，程序终止！请确认输入网址格式是否正确。");
			System.exit(1);
		}
		return null;// 不会被执行
	}
